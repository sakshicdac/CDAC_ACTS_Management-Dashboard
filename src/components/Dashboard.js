...
const Dashboard = () => {
    ...
    const [cityDistributionData, setCityDistributionData] = useState({});
    const [overallCourseData, setOverallCourseData] = useState({ labels: [], datasets: [] });
    const [overallGenderData, setOverallGenderData] = useState({ labels: [], datasets: [] });
    const [qualificationData, setQualificationData] = useState({ labels: [], datasets: [] });
   ...
   useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/gender-distribution")
        .then(response => {
            const transformedGenderData = response.data.reduce((acc, item) => {
                const genderId = item.gender || "Unknown"; // Default to "Unknown" if gender is missing
                const genderName = genderIdToNameMap[genderId]; // Map gender ID to name
            ...
            const genderChartData = {
                labels: transformedGenderData.map(item => item.gender),
                datasets: [{
                    data: transformedGenderData.map(item => item.count),
                    backgroundColor: ["#987D9A", "#BB9AB1"],
                },],
            };
            setOverallGenderData(genderChartData);
        })
        .catch(error => console.error("Error fetching overall gender distribution", error));
}, []); // Empty dependency array means this runs once on mount

// Fetch educational qualifications data
useEffect(() => {
    axios.get("http://localhost:5000/api/educational-qualifications")
        .then(response => {
            console.log(response.data); // Check what the response data looks like
            const transformedQualificationData = {
                labels: response.data.map(item => item.qual_course_name),
                datasets: [{
                    label: 'Qualifications',
                    data: response.data.map(item => item.qualification_count),
                    backgroundColor: ["#7495E3", "#94B3F2", "#AFE3C5", "#C9F0E2", "#F09EA7", "#F6CA94", "#FAFABE"],
                }],
            };
            setQualificationData(transformedQualificationData);
        })
        .catch(error => console.error("Error fetching educational qualifications", error));
}, []);
...
useEffect(() => {
    const fetchData = async () => {
        try {
            const genderResponse = await axios.get("http://localhost:5000/api/gender-distribution", {
                params: { course_id: selectedCourse, batch_id: selectedBatch, centre_id: selectedCentre }
            });
            const transformedGenderData = genderResponse.data.reduce((acc, item) => {
                const genderId = item.gender || "Unknown"; // Default to "Unknown" if gender is missing
                const genderName = genderIdToNameMap[genderId]; // Map gender ID to name
            ...
            const cityResponse = await axios.get("http://localhost:5000/api/city-wise-distribution", {
                params: { course_id: selectedCourse, batch_id: selectedBatch, centre_id: selectedCentre }
            });
            const cityCounts = cityResponse.data.reduce((acc, item) => {
                acc[item.city] = (acc[item.city] || 0) + item.student_count;
                return acc;
            }, {});
            ...
            setCityDistributionData(cityChartData);

            const courseResponse = await axios.get("http://localhost:5000/api/course-enrollment-distribution", {
                params: { course_id: selectedCourse, batch_id: selectedBatch, centre_id: selectedCentre }
            });
            const transformedCourseData = {
                labels: courseResponse.data.map(item => courseIdToNameMap[item.course_id]),
                datasets: [{
                    data: courseResponse.data.map(item => item.enrollment_count),
                    backgroundColor: ["#7495E3", "#94B3F2", "#AFE3C5", "#C9F0E2"],
                }],
            };
            setOverallCourseData(transformedCourseData);
        } catch (error) {
    ...
    fetchData();
}, [selectedBatch, selectedCentre, selectedCourse]);

// Filter data based on selected batch and update total students for that batch
useEffect(() => {
    if (selectedBatch) {
        const selectedBatchData = batchEnrollmentData.find(batch => batch.batch_id === selectedBatch);
        console.log("Selected Batch Data:", selectedBatchData);
        setSelectedBatchTotal(selectedBatchData ? selectedBatchData.total_students : 0);
    } else {
        setSelectedBatchTotal(0);
...
useEffect(() => {
    fetch('http://localhost:5000/api/city-wise-distribution')
        .then((response) => response.json())
        .then((data) => {
            setStudentData(Array.isArray(data) ? data : []); // Ensure studentData is an array
            setLoading(false);  // Set loading to false once data is fetched
        })
        .catch((error) => {
            console.error('Error fetching student data:', error);
    ...
    <div className="dashboard">
        ...
        <div className="content">
            ...
            {selectedBatch && (
                <>
                    <div className="chart-row">
                        ...
                        {selectedBatch && (
                            <div className="chart">
                                <h3>Batch: {batchIdToNameMap[selectedBatch]}</h3>
                                <div className="widget">
                                    <strong>Total Students Enrolled in {batchIdToNameMap[selectedBatch]}</strong>
                                    <p>{selectedBatchTotal}</p>
                                </div>
                            </div>
                        )}
                            ...
                            <div className="card" style={{ width: '350px' }}>
                                ...
                                {topCourse ? (
                                    <>
                                        <p>
                                            Course: <strong>{courseIdToNameMap[topCourse.course] || 'Unknown Course'}</strong>
                                        </p>
                                        <p>Enrollments: <strong>{topCourse.count}</strong></p>
                                    </>
                                ) : (
                            ...
                            <div className="chart" style={{ width: '450px' }}>
                                ...
                                <Bar data={qualificationData} options={{
                                    responsive: true,
                                    plugins: {
                                        legend: { position: 'top' },
                                        tooltip: { callbacks: { label(context) { return `${context.dataset.label}: ${context.raw}`; }, },
                                    ...
                                    scales: {
                                        x: { title: { display: true, text: 'Qualifications' } },
            ...
            <div className="overall-charts">
                ...
                <div className="chart">
                    <h3>Overall Gender Distribution</h3>
                    {overallGenderData && overallGenderData.datasets && overallGenderData.datasets.length > 0 ? (
                        <Pie data={overallGenderData} />) : (
                        <div>No data available</div>)}
                </div>
